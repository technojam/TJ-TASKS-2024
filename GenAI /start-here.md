# 🎓 **Generative AI Audition Tasks – The 8-Day Challenge**

Welcome to the **Generative AI Audition Challenge!** 🚀 Over these 8 days, you’ll dive into some exciting, hands-on mini-projects designed to test your **curiosity, creativity, and technical chops**. Whether you're a beginner or an advanced coder, this challenge offers something for everyone. It’s going to be **fun** but also **thought-provoking**—so get ready to stretch your brain a little!

Each task builds on practical aspects of **AI development** while giving you enough space to experiment. You’ll have **three levels of difficulty**, a **bonus challenge**, and a **fine-tuning task** of your choice—so choose your path wisely! 🧩

---

## 🗓 **Challenge Timeline**
- **Duration:** 8 Days
- **Task Submission:** Daily or Batch Submission by Day 8
- **Submission Format:** Code + Results + Optional Demo Video
- **Contact (Maintainer/Instructor):** [GitHub: anurag-bit](https://github.com/anurag-bit)

---

## 🛠 **Tools & Languages**
- **Languages:** Python, JavaScript, or any language you're comfortable with
- **Libraries:** TensorFlow, PyTorch, Hugging Face, OpenAI, or others
- **Platforms:** Google Colab, Kaggle, Jupyter Notebooks
- **Other:** GitHub/GitLab for code submission

---

# 🥳 **Project Levels and Tasks**

---

## 🌟 **Beginner Level (Super Easy)** (Day 1)
### **Task: Random Poetry Generator**
**Description:** Write a script that randomly selects words from a list (nouns, verbs, adjectives, etc.) and generates a short **poem** or **haiku**. Add a bit of formatting magic to make it look fancy! 🎩

**Example Output:**
A dancing river
Swirls under skies of gold light
Whispers echo loud

markdown
Copy code

**Deliverables:**
1. Python/JavaScript code generating at least **5 unique poems**
2. Include formatting touches for creativity points
3. Submit code on GitHub

**Grading Parameters:**
- **Creativity:** (10 points)
- **Correctness of Code:** (10 points)
- **Bonus:** Fancy formatting or unexpected poetry theme (5 points)

---

## 🚀 **Level 1: Easy** (Day 2)
### **Task: Text Generator Using Markov Chains**
**Description:** Build a **simple text generator** using Markov Chains. Use a text corpus of your choice (Shakespeare’s plays, song lyrics, etc.) and generate short but coherent sentences.

**Deliverables:**
1. Markov Chain model implementation
2. 5 samples of generated sentences
3. Short explanation of how Markov Chains were used

**Grading Parameters:**
- **Correctness:** (10 points)
- **Completeness:** (5 points)
- **Creativity of Text Outputs:** (5 points)

---

## 🌱 **Level 2: Intermediate** (Day 3-5)
### **Task: AI Image Caption Generator**
**Description:** Create a pipeline that takes an **image** as input and generates a **caption** for it using a pre-trained model (e.g., InceptionNet + LSTM). Try using **MS COCO samples** or your own image collection.

**Deliverables:**
1. Image captioning model pipeline
2. Captions for at least **3 images**
3. Short write-up about how your model works

**Grading Parameters:**
- **Accuracy of Captions:** (15 points)
- **Technical Pipeline:** (10 points)
- **Creativity & Error Handling:** (5 points)

---

## 🔥 **Level 3: Advanced** (Day 6-7)
### **Task: Fine-tune a Language Model (Choice-based)**
**Description:** Choose a small GPT model (or BERT variant) and **fine-tune** it on a niche dataset of your choice (e.g., movie dialogues, poetry, or even cooking recipes!). The goal is to make the model generate contextually relevant responses.

**Deliverables:**
1. Fine-tuned model (via GitHub or Colab)
2. Model performance metrics (e.g., BLEU, Perplexity)
3. Sample outputs with **explanation of limitations**

**Grading Parameters:**
- **Performance:** (20 points)
- **Explanation of Results & Challenges:** (10 points)
- **Creativity of Dataset/Theme:** (5 points)

---

## 🏅 **Bonus Challenge: Generative Artwork**
**Description:** Use **Stable Diffusion, DALL·E, or VQGAN + CLIP** to create digital artwork around a theme of your choice (e.g., “The Future of AI,” “Dreamscapes,” etc.). Include a **short description** explaining your inspiration. 🎨

**Deliverables:**
1. At least **2 artworks**
2. Concept description (100-150 words)
3. Technical process overview

**Grading Parameters:**
- **Visual Appeal & Theme Relevance:** (20 points)
- **Technical Documentation:** (10 points)
- **Creative Explanation:** (10 points)

---

## 📊 **Grading System**

Each task will be evaluated on the following **scoring rubric** (out of 100):

| **Criteria**                | **Weight** |
|------------------------------|------------|
| Correctness & Completeness   | 30 points  |
| Creativity & Innovation      | 20 points  |
| Technical Complexity         | 20 points  |
| Documentation & Presentation | 20 points  |
| Timeliness of Submission     | 10 points  |

### **Bonus Points:**
- **5-10 extra points** for **outstanding creativity**
- **10 extra points** for completing the **bonus challenge**

---

## 🎯 **Final Grading Tiers**
- **90+ points:** 🥇 Outstanding! You’ve nailed it and are ready to take on real-world AI challenges.
- **75-89 points:** 🥈 Great job! You’ve demonstrated strong skills and creativity.
- **60-74 points:** 🥉 Good effort! A little more practice and you’ll be there.
- **Below 60:** ❗ Needs improvement—don’t give up! Keep practicing and try again.

---

## 📚 **Resources for Learning**

Here are some **useful resources** to help you with the tasks:

1. **Python for AI Basics:**
   - [Python Official Docs](https://docs.python.org/3/)
   - [Python Crash Course on Codecademy](https://www.codecademy.com/learn/learn-python-3)

2. **Generative AI with Hugging Face:**
   - [Hugging Face Course](https://huggingface.co/course)
   - [Transformers Documentation](https://huggingface.co/docs/transformers/index)

3. **Markov Chains:**
   - [Markov Chain Tutorial](https://brilliant.org/wiki/markov-chains/)

4. **Image Captioning Models:**
   - [MS COCO Dataset](https://cocodataset.org/#home)
   - [Image Captioning Tutorial](https://machinelearningmastery.com/develop-a-captioning-model-in-keras/)

5. **Fine-tuning GPT/BERT Models:**
   - [Fine-tuning BERT](https://huggingface.co/docs/transformers/training)
   - [Google Colab: GPT Fine-Tuning](https://colab.research.google.com/)

6. **AI Artwork & Creativity Tools:**
   - [DALL·E by OpenAI](https://openai.com/dall-e)
   - [VQGAN + CLIP Guide](https://www.assemblyai.com/blog/how-to-generate-images-using-vqgan-and-clip/)

---

## 📤 **Submission Guidelines**
1. **GitHub Repository:** Submit a **GitHub/GitLab link** with your code and documentation.
2. **Video Demo (Optional):** Share a **3-5 minute video** explaining your work.
3. **Deadline:** Ensure all tasks are submitted by **the end of Day 8**.

---

## 🏁 **Final Thoughts**

This audition is **more than just a test**—it’s a chance to explore your passion for generative AI. Experiment, be curious, and don’t be afraid to try something new! We can’t wait to see the amazing things you’ll create.

---

## 📞 **Maintainer & Instructor Contact**
For any queries or assistance, reach out:
- **GitHub:** [anurag-bit](https://github.com/anurag-bit)
- **Email:** [singh.ask@outlook.com](mailto:singh.ask@outlook.com)

---

Good luck, and happy coding! 🎉
